...
interface ConfInfo {
    conf: ConfData | undefined;
}

export const ConferencePageView: React.FC<ConfInfo> = ({conf}) => {
    document.title = conf?.name ?? "Конференция";
    ...
    if (conf && conf.isFounded) {
        return (
            <Grid container spacing={1}>
                <ConferenceTitle title={conf.name} userRoles={role} confId={conf.pageLink}/>
                ...
                <FullDescription description={conf.description}/>
